    ...
    def signIn_button(self):
        ...
        if uChekck_data != 0:
            ...
            if int(uPhoneChekck_data) >= 3:
                ...
                # 注册开始
                signIn_SQL = "INSERT INTO User_CZW(id, USER, PASSWORD, NAME, phone, time, activateStatus, createTime, days) VALUES ('{0}', '{1}', '{2}', '{3}', '{4}', date_add(CURDATE(), INTERVAL {5} DAY), 1, NOW(), {5} )".format(
                ...
                signIn_data = linkDatabase.cur_insert(None, signIn_SQL)
                uesrConfiguration_data = 0
                if signIn_data == 1:
                    uesrConfiguration_data = linkDatabase.cur_insert(None, uesrConfiguration_SQL)
                else:
